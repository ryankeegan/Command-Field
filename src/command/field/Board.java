...
public class Board {
    ...
    public static void Draw(Graphics2D g) {
        ...
        for (int i=0; i<NUM_ROWS; i++) {
            g.setColor(Color.black);
            g.setFont(new Font("Arial", Font.PLAIN, 5));
            g.drawString(""+i, (Window.getX(0)+xdelta/2)+5, Window.getY(i*ydelta)+ydelta/2+(Window.getActualWindowHeight()-BoardHeight));
        }
    }
    
    public static void BoardInit() {
        Reset();
        int seed = (int)(Math.random()*(0x7fffffff));
    ...
    public static void CheckCollision(int _xPos, int _yPos) {
        if (_xPos > Window.getX(xdelta) && _yPos > Window.getY((Window.getActualWindowHeight()-BoardHeight)+ydelta) && _xPos < Window.getX(xdelta*NUM_COLUMNS) && _yPos < Window.getY(ydelta*(NUM_ROWS+2))) {
